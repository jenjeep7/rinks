...
function initializeUser() {
  pg.connect(connectionString, function(err, client, done){
    if(err) {
      ...
      var query = client.query('CREATE TABLE IF NOT EXISTS users (id serial PRIMARY KEY, username varchar(80) NOT NULL, password varchar(80) NOT NULL)');
      query.on('end', function(){
...
function initializeReviewsDB(){
  pg.connect(connectionString, function(err, client, done){
    if (err) {
      ...
      'cold_rating varchar(5) NOT NULL,');
